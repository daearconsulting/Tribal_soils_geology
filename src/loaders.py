...
def load_wsd_horizon_raster(unit_name: str) -> object | None:
    ...
    try:
        ...
        if not gdb_path.exists():
            ...
            return None
        # Rasters in GDB use the unit name as the layer identifier
        # Exact layer names are in DescriptionOfModelUnits
        ds = rasterio.open(f"OpenFileGDB:{gdb_path}:{unit_name}")
        return ds
    except Exception as e:
        warnings.warn(
            f"Could not open horizon raster '{unit_name}': {e}. "
            "Check that the unit name matches DescriptionOfModelUnits exactly.",
            UserWarning, stacklevel=2,
        )
        return None
    ...
    for gdb in gdbs:
        try:
            ...
            if comp_layer:
                ...
                parts.append(pd.DataFrame(gdf.drop(columns="geometry",
                                                    errors="ignore")))
        except Exception as e:
...
def load_tribal_soil_profiles(
    ...
    See data/templates/soil_profile_template.xlsx for the expected format.
    ...
    if path is None:
        warnings.warn(
            ...
            "See data/templates/soil_profile_template.xlsx to begin "
...
def load_tribal_well_logs(
    ...
    This data is GITIGNORED and stays under Tribal control.
    See data/templates/well_log_template.xlsx for the expected format.
    """
    ...
    if path is None:
        warnings.warn(
            ...
            "See data/templates/well_log_template.xlsx.",
            UserWarning, stacklevel=2,
        )
